"""Command-line interface and scan orchestration for AVS."""
...
from core.models import EvidenceStep, Severity
...
from scanners.crypto import CryptographyScanner
...
def dedup_findings(findings):
    ...
    for bucket in grouped.values():
        ...
        for note in other_notes[:5]:
            all_evidence.append(
                EvidenceStep(kind="NOTE", description="Also observed in", notes=note)
            )
...
def _dedup_identical_findings(findings):
    ...
    for finding in findings:
        key = (
            ...
            finding.component_name, finding.entrypoint_method,
            finding.recommendation, tuple(finding.references),
